...
class Role extends Model {
	...
	$x.=stylesheet_formate(TP_BACK.'plugins/bootstrap-select/bootstrap-select.min.css');
...
public static function hd_script() {
	$x=script_formate('assets/js/jquery.slugify.js'); 	
	$x.=script_formate(TP_BACK.'plugins/bootstrap-select/bootstrap-select.min.js');
  ...
  swalWithBootstrapButtons({
    ...
    protected static function action_data($id, $type)
    {
        ...
        if (isset($_REQUEST['submit']))
        {
            ...
            if ($type == 'edit')
            {
				...
				$ro=Permission::whereIn('id', $permission)->get();
				$data->permissions()->sync($ro);               
                $pp = $data->save();
    ...
    public static function form_data()
    {
        ...
        else
        {
            ...
            $slug = $rw->slug;
			$permission = Permission::pluck('name','id')->toArray();
        	$myper = $rw->permissions->pluck('id')->toArray();   
        }

        echo $fo = Forms::input("Name","cname", $cname,1);
        echo $fo = Forms::input("Slug Name","slug", $slug,1);      
 		echo $fo = Forms::Select_multi("User Level","permission",$permission,$myper,1);
